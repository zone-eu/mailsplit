...
class Headers {
    ...
    addFormatted(key, line, index) {
        ...
        let header = {
            key: this._normalizeHeader(key),
            line: this._normalizeLineBreaks(line, '\r\n')
        };
    ...
    build(lineEnd) {
        ...
        let headers = lines
            .map(line => this._buildHeaderLine(this._normalizeLineBreaks(line.line, ending)))
    ...
    _normalizeLineBreaks(line, lineEnd) {
        return (
            line
                // build() joins lines with lineEnd, so a line starting with a line break would
                // close the header block, and one starting with whitespace would fold itself
                // into the header before it
                .replace(/^[\r\n \t]+/, '')
