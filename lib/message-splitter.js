...
// everything a boundary delimiter line may hold besides the boundary value itself.
// compareBoundary() accepts up to boundary.length + 6 + startpos, and checkBoundary()
// allows startpos to be 2, so this is the longest a delimiter line can ever be.
const BOUNDARY_LINE_OVERHEAD = 2 /* leading <CR><LF> */ + 2 /* "--" prefix */ + 2 /* "--" suffix */ + 2; /* trailing <CR><LF> */
...
/**
 * Moves an end offset back over the line ending that closes a body line. The line
 * ending in front of a boundary belongs to the delimiter, not to the part content.
 *
 ...
 */
function trimBodyLineEnd(chunk, start, end) {
...
class MessageSplitter extends Transform {
    ...
    _transform(chunk, encoding, callback) {
        ...
        let iterateData = () => {
                        ...
                        if (flush) {
                            if (group && group.type !== 'none') {
                                if (group.type === 'body' && group.node && group.node.parentNode) {
                                    ...
                                    groupend = trimBodyLineEnd(chunk, groupstart, groupend);
                                }
                                ...
                                if (groupstart < groupend) {
                                    // always re-slice, the value the line was emitted with
                                    // may still include the line ending we just trimmed
            ...
            if (group.type === 'body' && group.node && group.node.parentNode) {
                // do not include the last line ending for body
                pos = trimBodyLineEnd(chunk, groupstart, pos);
            }
            ...
            if (pos < chunk.length) {
                this.appendPendingLine(chunk.slice(pos));
            }
     ...
     * body content, in which case it is pushed out rather than held in memory.
     *
    ...
    enforcePendingLineLimit() {
        ...
        if (this.state === HEAD) {
            ...
            if (this.node._headerlen + this.lineLength > this.maxHeadSize) {
    ...
    processLine(line, final, next) {
        ...
        let truncatedLine = this.skipBoundaryCheck;
        this.skipBoundaryCheck = false;
        ...
        switch (this.state) {
            case HEAD: {
                ...
                if (this.node._headerlen > this.maxHeadSize) {
                    return next(maxLenError('Max header size for a MIME node exceeded'));
                }
